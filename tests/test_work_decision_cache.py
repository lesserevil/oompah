...
def test_real_controller_commit_failure_rolls_back_before_publication(
    tmp_path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config = orchestrator_module.ServiceConfig(
    ...
    orchestrator._notify_observers = Mock()
    orchestrator._notify_state_only = Mock()
    previous_controller = orchestrator.workflow_controller.health_snapshot()
    previous_snapshot = orchestrator.work_decision_snapshot()
    monkeypatch.setattr(
        orchestrator.workflow_job_store,
    ...
    assert result["publication_accepted"] is False
    assert result["publication_rejection"] == "durable_commit_failed"
    assert orchestrator.workflow_controller.health_snapshot() == previous_controller
    assert orchestrator.work_decision_snapshot() == previous_snapshot
    orchestrator._notify_observers.assert_not_called()
    orchestrator._notify_state_only.assert_not_called()
...
def test_controller_scan_limit_rotates_coverage_and_never_publishes_omissions(
    tmp_path,
) -> None:
    config = orchestrator_module.ServiceConfig(
        workspace_root=str(tmp_path / "workspace")
    ...
    first_tasks = {item["task_id"] for item in first_snapshot["items"]}
    second_tasks = {item["task_id"] for item in second_snapshot["items"]}
    assert first_result["truncated"] is True
    assert first_result["omitted_tasks"] == 1
    assert second_result["truncated"] is True
    assert second_result["omitted_tasks"] == 1
    assert first_snapshot["availability"] == "incomplete"
    assert second_snapshot["availability"] == "incomplete"
    assert first_snapshot["complete"] is False
    assert second_snapshot["complete"] is False
    assert first_tasks | second_tasks == {"TASK-1", "TASK-2", "TASK-3"}
    first_omitted = first_snapshot["incomplete_tasks"][0]["task_id"]
    second_omitted = second_snapshot["incomplete_tasks"][0]["task_id"]
    assert first_omitted != second_omitted
    assert orchestrator.work_decision_projection(
        "project-a", second_omitted
    ) is None
    assert orchestrator.work_decision_availability(
        "project-a", second_omitted
    ) == "incomplete"
