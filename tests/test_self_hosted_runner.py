...
class TestDedicatedCiWorkflowRunsOnLabels:
    """Dedicated CI workflow must target the self-hosted oompah runner."""
    ...
    def test_ci_test_job_targets_self_hosted(self):
        ...
        assert "self-hosted" in labels, (
            f"ci-dedicated.yml 'test' job must include 'self-hosted' in runs-on; got {labels}"
        )
    ...
    def test_ci_test_job_targets_oompah_label(self):
        ...
        assert "oompah" in labels, (
            f"ci-dedicated.yml 'test' job must include 'oompah' in runs-on; got {labels}"
        )
    ...
    def test_ci_test_job_includes_all_required_labels(self):
        ...
        assert not missing, (
            f"ci-dedicated.yml 'test' job missing labels: {missing}. Current runs-on: {labels}"
        )
    ...
    def test_ci_test_job_does_not_use_github_hosted_runner(self):
        ...
        assert not overlap, (
            f"ci-dedicated.yml 'test' job must not target GitHub-hosted runners; found {overlap}. "
    ...
    def test_all_ci_jobs_target_required_labels(self):
        """Every job in ci-dedicated.yml must use the required self-hosted labels.
        ...
        assert not failures, (
            "All ci-dedicated.yml jobs must target the self-hosted oompah runner:\n"
            + "\n".join(failures)
        )

    def test_no_ci_job_uses_github_hosted_runner(self):
        """No job in ci-dedicated.yml may use a GitHub-hosted runner name.
        ...
        assert not violations, (
            "ci-dedicated.yml must not use GitHub-hosted runners:\n" + "\n".join(violations)
        )
