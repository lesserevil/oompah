...
class TestWorkerFailureLogging:
    """Verify worker failures are logged at DEBUG level to avoid error_watcher.
    
    Regression tests for OOMPAH-1200: ACP/API/CLI worker failures with expected
    pre-request exceptions should not be captured by error_watcher.
    """

    def test_debug_level_logs_not_captured_by_handler(self):
        """DEBUG level logs should not be captured by ERROR-level handler."""
        tracker = MagicMock()
        watcher = ErrorWatcher(tracker)
        watcher.install_log_handler("oompah.test_debug_worker")
        
        try:
            test_logger = logging.getLogger("oompah.test_debug_worker")
            
            # Log a DEBUG message (what worker failures should use)
            test_logger.debug("Worker failed with transient error")
            
            # Log an ERROR message (what should be captured)
            test_logger.error("Worker failed with fatal error")
            
            # The handler should only capture ERROR, not DEBUG
            assert tracker.create_issue.call_count == 1
            created_issue = tracker.create_issue.call_args
            assert "fatal error" in str(created_issue).lower()
            
        finally:
            watcher.uninstall_log_handler("oompah.test_debug_worker")

    def test_exception_with_error_keyword_in_debug_not_captured(self):
        """Exception messages containing ERROR should be safe at DEBUG level."""
        tracker = MagicMock()
        watcher = ErrorWatcher(tracker)
        watcher.install_log_handler("oompah.test_exc_debug")
        
        try:
            test_logger = logging.getLogger("oompah.test_exc_debug")
            
            # Log a DEBUG message with "ERROR" in the text
            # (simulating exception message containing "ERROR")
            test_logger.debug(
                "Worker failed: %s",
                "ERROR: transient workspace issue",
                extra={"issue_id": "test-123"}
            )
            
            # Log an ERROR message
            test_logger.error(
                "Worker failed: %s",
                "CRITICAL: fatal workspace issue",
                extra={"issue_id": "test-124"}
            )
            
            # Only the ERROR message should create a task
            assert tracker.create_issue.call_count == 1
            created_issue = tracker.create_issue.call_args
            assert "fatal" in str(created_issue).lower()
            
        finally:
            watcher.uninstall_log_handler("oompah.test_exc_debug")
