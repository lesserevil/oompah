...
def test_gate_subprocess_isolates_operator_and_tool_state(tmp_path, monkeypatch):
    ...
    assert trusted_home.name.startswith("oompah-quality-gate-trusted-")
    assert run_root.name.startswith("oompah-quality-gate-")
    ...
    assert values[7] == str(trusted_home / "pytest-workers" / "session")
    assert not trusted_home.exists(), "trusted HOME survived gate cleanup"
    assert not run_root.exists(), "candidate run root survived gate cleanup"
...
def test_gate_allocation_failure_releases_lease_and_generation(
    ...
    assert all(
        not BranchQualityGate._gate_root_owner_path(root).exists()
        for root in created
    )
...
def test_gate_startup_scavenges_roots_from_dead_service_generation(
    ...
    roots = (run_root, trusted_home)
    try:
        for root in roots:
            BranchQualityGate._forget_gate_root(root)
            owner_path = BranchQualityGate._gate_root_owner_path(root)
        ...
        BranchQualityGate(str(run_root.parent / "scavenge-state.json"))

        assert all(not root.exists() for root in roots)
    finally:
        for root in roots:
            if root.exists():
                if root.name.startswith(quality_gate._TRUSTED_GATE_ROOT_PREFIX):
                    BranchQualityGate._cleanup_gate_trusted_home_root(root)
                else:
                    BranchQualityGate._cleanup_gate_run_root(root)
            BranchQualityGate._gate_root_owner_path(root).unlink(missing_ok=True)
...
def test_gate_scavenger_retains_old_root_with_exact_live_owner(
    ...
    run_root = BranchQualityGate._gate_run_root()
    try:
        owner_path = BranchQualityGate._gate_root_owner_path(run_root)
        owner_path.chmod(0o600)
...
def test_gate_scavenger_retains_root_when_proc_identity_is_unknown(
    ...
    monkeypatch.setattr(quality_gate.tempfile, "tempdir", str(tmp_path))
    run_root = BranchQualityGate._gate_run_root()
    BranchQualityGate._forget_gate_root(run_root)
    monkeypatch.setattr(
        BranchQualityGate,
...
def test_gate_restart_scavenges_abandoned_quarantine_and_sidecar(
    ...
    quarantine = run_root.with_name(
        f".{run_root.name}.scavenge-2000000000-123456789"
    )
...
def test_gate_normal_cleanup_removes_both_owner_sidecars(tmp_path, monkeypatch):
    monkeypatch.setattr(quality_gate.tempfile, "tempdir", str(tmp_path))
    run_root = BranchQualityGate._gate_run_root()
    trusted_home = BranchQualityGate._gate_trusted_home_root(run_root)
    sidecars = (
        BranchQualityGate._gate_root_owner_path(run_root),
        BranchQualityGate._gate_root_owner_path(trusted_home),
    )
