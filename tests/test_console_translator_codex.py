"""Tests for the Codex-backend console translator
(oompah-zlz_2-elug, Console 6/6).

Parallels :mod:`tests.test_console_translator_claude` — same per-kind
mapping coverage plus codex-specific SDK-history shape assertions.
"""
...
from oompah.console_translators.codex import (
    acp_to_normalized,
...
def _ev(event: str, payload: dict | None = None, usage: dict | None = None) -> AgentEvent:
    """Build an AgentEvent matching what codex.py emits."""
    return AgentEvent(
        event=event,
...
class TestKindMapping:
    """Every acp_* event from the codex backend should produce a
    backend='codex' stamped ConsoleEvent of the right kind."""
    ...
    def test_acp_session_start_to_session_meta(self):
        ev = _ev(
            "acp_session_start",
            {
                ...
                "tool_catalog": ["read_file", "write_file"],
        ...
        assert norm.args["billing_model"] == "per_token"
    ...
    def test_acp_text_to_agent_text(self):
        ...
        assert norm.backend == "codex"
        assert norm.text == "hi from codex"
    ...
    def test_acp_tool_use_to_tool_call(self):
        ev = _ev(
            "acp_tool_use",
            {"tool": "read_file", "input": {"path": "AGENTS.md"}, "id": "tu_42"},
        )
        norm = acp_to_normalized(ev)
        assert norm.kind == "tool_call"
        assert norm.backend == "codex"
        assert norm.tool == "read_file"
        assert norm.args == {"path": "AGENTS.md", "_tool_use_id": "tu_42"}
...
class TestBackendStamp:
    """Every translated codex event must stamp backend='codex'."""
    ...
    def test_every_kind_has_backend_stamp(self):
        ...
        for evt_name, payload in kinds:
            ...
            assert norm.backend == "codex", f"missing codex stamp for {evt_name}"
...
class TestSdkHistory:
    """The codex history shape is a list of openai-agents input items:
    ...
    def test_operator_input_with_attachments_inlines_paths(self):
        ...
        # Content carries both the text and one line per attachment.
    ...
    def test_agent_text_becomes_assistant_message(self):
        events = [
            ...
            ConsoleEvent(ts="t2", kind="agent_text", backend="codex", text="here you go"),
        ]
    ...
    def test_empty_agent_text_produces_no_item(self):
        events = [
            make_operator_input("t1", "go"),
            ConsoleEvent(ts="t2", kind="agent_text", backend="codex", text=""),
        ]
        history = normalized_to_sdk_history(events)
    ...
    def test_tool_call_becomes_function_call_item(self):
        ...
        history = normalized_to_sdk_history(events)
        # user message, then function_call item.
        assert history[0] == {"role": "user", "content": "read README"}
        call = history[1]
        assert call["type"] == "function_call"
        assert call["name"] == "read_file"
        assert call["call_id"] == "tu_1"
        # Arguments must be a JSON-encoded string per the SDK shape.
    ...
    def test_tool_call_with_raw_input_string(self):
        """A truncated string ``_raw_input`` is passed through as the
    ...
    def test_thinking_permission_session_meta_error_skipped(self):
        events = [
            ...
            ConsoleEvent(ts="t4", kind="session_meta", model="gpt-4"),
    ...
    def test_alternating_roles_allowed(self):
        """openai-agents accepts adjacent same-role messages (unlike
        Anthropic). Two consecutive agent_text events should produce
    ...
    def test_round_trip_full_conversation(self):
        ...
        SDK history produces an input-item list the codex SDK can
        consume verbatim."""
        events_in = [
            _ev("acp_session_start", {"model": "gpt-4o-mini"}),
        ...
        # Shape assertions: every item must be a dict with either
        # `role` (message) or `type` (function_call*).
        for item in history:
        ...
        # All four "model-input" turns should be present (the
        # session_start / session_meta / result items are excluded).
        assert len(history) == 5  # user + assistant + call + output + assistant
...
class TestDispatch:
    def test_codex_in_known_backends(self):
        assert "codex" in known_backends()

    def test_get_translator_codex(self):
        assert get_translator("codex").backend == "codex"

    def test_explicit_codex_dispatch_matches_direct(self):
        ev = _ev("acp_text", {"text": "x"})
        direct = acp_to_normalized(ev)
        dispatched = dispatch_acp_to_normalized(ev, backend="codex")
    ...
    def test_history_dispatch_matches_direct(self):
        ...
        dispatched = dispatch_normalized_to_sdk_history(events, backend="codex")
        assert direct == dispatched
...
class TestNormalizedRoundTripViaJson:
    """Every codex event kind round-trips cleanly through dict form."""
