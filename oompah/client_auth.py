...
# ---------------------------------------------------------------------------
# Quality gate isolation (server-controlled launch boundary)
# ---------------------------------------------------------------------------
...
def quality_gate_environment(
    ...
    """Return an environment with full lifecycle isolation for quality gate execution.

    A quality gate runs in a disposable worktree while the operator's service
    may be alive in another checkout. This function configures the environment
    to enforce isolation at the launch boundary, preventing any candidate code
    from discovering or signaling the operator service, regardless of whether
    the candidate Makefile/scripts cooperate or predate the isolation variables.

    Enforced isolation:
    * Private temporary root (TMPDIR, TMP, TEMP) — prevents /tmp quota issues
      and log file discovery
    * Ephemeral server port (OOMPAH_TEST_SERVER_PORT) — prevents port conflicts
    * Private PID files (OOMPAH_TEST_PID_FILE, .meta) — prevents canonical
      lifecycle file discovery
    * Removed server URL and credentials — prevents connection to operator
    * Process group / session tracking — enables exact ownership cleanup

    The quality gate subprocess is started with start_new_session=True, which
    creates a new process group. This function does not modify the environment's
    HOME or other state; the subprocess's isolated temp root and PID paths
    provide the essential containment.

    Args:
        base_env: Optional environment dict to use instead of os.environ.
                 Defaults to current process environment.

    Returns:
        A new environment dict safe to pass to subprocess.Popen() for a
        candidate command, with all lifecycle isolation enforced at this
        server-controlled boundary before the command executes.

    Raises:
        OSError: If unable to create the private temp directory or allocate
                 an ephemeral port.
    """
    import socket
    import tempfile
    ...
    # Start with a clean agent environment (removes client credentials).
    environment = agent_environment(base_env)

    # Create a private, disposable temp directory for this specific quality gate run.
    # This ensures the gate cannot inherit or access the operator's /tmp or HOME state.
    # Use the operator's configured OOMPAH_TEMP_ROOT as the parent if available.
    temp_root_parent = environment.get("OOMPAH_TEMP_ROOT", "~/.oompah/tmp")
    if temp_root_parent.startswith("~"):
        temp_root_parent = os.path.expanduser(temp_root_parent)
    temp_root_parent = os.path.abspath(temp_root_parent)

    pytest_temp_parent = Path(temp_root_parent) / "pytest"
    pytest_temp_parent.mkdir(mode=0o700, parents=True, exist_ok=True)
    pytest_run_root = tempfile.mkdtemp(
        prefix="run.",
        dir=str(pytest_temp_parent),
        # tempfile defaults to mode 0o700 for mkdtemp
    )
    lifecycle_root = os.path.join(pytest_run_root, "lifecycle")
    os.makedirs(lifecycle_root, mode=0o700, exist_ok=True)

    # Allocate an ephemeral port: bind to 0, the kernel assigns an available port,
    # then immediately release it. The gate will use this port in its lifecycle setup.
    # This approach follows scripts/run-tests.sh convention.
    ephemeral_port = 0
    try:
        ...
        # Clean up the temp directory if port allocation fails.
        try:
            import shutil
            shutil.rmtree(pytest_run_root)
        except OSError:
            pass
        raise OSError(f"Cannot allocate ephemeral port for quality gate: {exc}") from exc

    # Enforce lifecycle isolation variables so a candidate Makefile cannot
    # discover the operator's .oompah.pid or connect to the operator's port.
    environment["OOMPAH_PYTEST_GATE"] = "1"
    environment["OOMPAH_PYTEST_RUN_ROOT"] = pytest_run_root
    environment["OOMPAH_TEST_SERVER_PORT"] = str(ephemeral_port)
    environment["OOMPAH_SERVER_PORT"] = str(ephemeral_port)
    environment["OOMPAH_TEST_PID_FILE"] = os.path.join(
        lifecycle_root, ".oompah.pid"
    )
    environment["OOMPAH_TEST_PID_META_FILE"] = os.path.join(
        lifecycle_root, ".oompah.pid.meta"
    )

    # Redirect all temporary file creation to the private run root.
    # This prevents subprocess temp files from polluting the operator's /tmp.
    environment["TMPDIR"] = pytest_run_root
    environment["TMP"] = pytest_run_root
    environment["TEMP"] = pytest_run_root

    # Remove server URL and any task-specific capability so a candidate
    # cannot connect to the operator service even if it were listening.
    environment.pop("OOMPAH_SERVER_URL", None)
    environment.pop("OOMPAH_TASK_HANDOFF_TOKEN", None)
    environment.pop("OOMPAH_TASK_HANDOFF_PROJECT_ID", None)
