...
@dataclass(frozen=True, slots=True)
class PreparedControllerPass:
    """Pure decisions collected before the durable publication transaction."""
...
class UniversalTotalityLivenessController:
    ...
    def publication_transaction(self) -> Iterator[Any]:
        """Stage controller metrics and durable scheduling until publication.

        The yielded token has a ``commit()`` method. Without that explicit
        commit, the controller, scheduler, generation/window cursors, schedule
        cursors, and jobs are restored to their exact prior cut.
        """

        with self._lock, self.scheduler._metrics_lock:
            controller_state = (
                self._evaluated,
                self._passes,
                self._escalated,
            ...
            scheduler_state = (
                self.scheduler._reconciled_decisions,
                self.scheduler._stale_decisions,
                self.scheduler._last_error,
            )
            with self.store.publication_transaction() as durable:
                class _ControllerPublication:
                    committed = False
                ...
                transaction = _ControllerPublication()
                try:
                    yield transaction
                finally:
    ...
    def evaluate(
        ...
        *,
        ...
        """Evaluate one bounded rotating window of non-final tasks."""
        ...
        # Fact collection is deliberately unbounded by the durable window here.
        # The publication phase chooses its bounded rotating window after all
        # arbitrary/slow providers have returned, so no SQLite write transaction
        # or workflow-job lease is held while collecting evidence.
        ...
        if _record_metrics and len(selected) > self.decision_limit:
            offset = self.store.allocate_decision_window(
                total=len(selected), limit=self.decision_limit
            )
    ...
    def prepare(
        ...
        *,
        ...
        facts_by_task: Mapping[Any, WorkflowFacts] | None = None,
        now: datetime | None = None,
    ) -> PreparedControllerPass:
        """Collect and decide all live facts without durable store mutation."""
        ...
        return PreparedControllerPass(decisions)

    def publish_prepared(
        self,
        prepared: PreparedControllerPass,
        *,
        snapshot_generation: int | None = None,
    ) -> ControllerPass:
        """Atomically choose a bounded window and materialize its recovery work."""

        if not isinstance(prepared, PreparedControllerPass):
            raise TypeError("prepared must be a PreparedControllerPass")
        ...
        ordered = list(prepared.decisions)
        ...
        result = ControllerPass(
            ...
            reconciliation,
            escalations,
            reconciliation.truncated or truncated,
        )
        with self._lock:
    ...
    def reconcile(
        ...
        *,
        ...
        with self.publication_transaction() as publication:
            result = self.publish_prepared(
                prepared, snapshot_generation=snapshot_generation
            )
            publication.commit()
        return result
