---
id: OOMPAH-17
type: epic
status: Archived
priority: 1
title: Define and harden the 1.0 release train
parent: OOMPAH-16
children:
- OOMPAH-18
- OOMPAH-19
- OOMPAH-20
- OOMPAH-21
blocked_by: []
labels:
- release:1.0
assignee: null
created_at: '2026-06-22T01:14:45.345931Z'
updated_at: '2026-06-29T14:44:09.923405Z'
work_branch: epic-OOMPAH-17
target_branch: epic-OOMPAH-16
review_url: https://github.com/lesserevil/oompah/pull/344
review_number: '344'
merged_at: null
oompah.review_url: https://github.com/lesserevil/oompah/pull/344
oompah.review_number: '344'
oompah.work_branch: epic-OOMPAH-17
oompah.target_branch: epic-OOMPAH-16
---
## Summary

Plan: plans/oompah-1.0-release.md#epic-a-define-and-harden-the-10-release-train

WHAT TO DO
Prepare the repository for the release/1.0 branch, force-movable v1.0.0-draft release-candidate tags, and immutable v1.0.0 final tags.

DONE WHEN
CI covers release branches, release automation understands draft tags without weakening final validation, and the release branch cut checklist is documented.

## Acceptance Criteria

- [ ] Define acceptance criteria.

## Notes

