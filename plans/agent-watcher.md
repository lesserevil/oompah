...
> auto-files deferred beads for misbehavior patterns. It has not been
...
- Agents past turn 40 still working, no `bd close` in sight
- Agents that committed and pushed but never reached `bd close`
...
dispatch budget gets burned silently. Today the only auto-bead path is
...
| **Dedup grain** (Q2) | Per-fingerprint across all time. Each bead carries a `note` listing the latest 5 occurrences (issue id, turn, ts). |
...
| **commit_without_close source** (Q4) | Both: tool stream check (did the agent call `bd close`?) AND `bd show <id>` re-query at end of run (is it actually closed — possibly by the operator via UI). Only file the bead when both say no. |
| **Annotate original or new bead** (Q5) | Both. Add a comment on the original issue so the next agent dispatched on it sees the prior misbehavior, AND file a new deferred bead for the operator-facing fix. |
...
stream, files one deferred bead per *unique* fingerprint, with the existing
issue annotated.
...
| `commit_without_close` | `git commit` + `git push` succeed but no `bd close` before run end (only flag for `Worker completed normally`, AND `bd show` confirms not-closed) | 1 |
...
| `over_turn_budget` | Run hit `max_turns` without `bd close` | 2 |
...
So the same loop pattern across 50 runs files 1 deferred bead, not 50.
Each new occurrence within the window just appends to the bead's `note`
and refreshes `last_seen`. Outside the window, a new bead is filed.
...
tracker.add_comment(
    ...
    f"Evidence: {short_excerpt}. See bead {filed_bead.identifier}.",
    author="oompah",
)
```
...
- `oompah/agent_watcher.py` — new. `AgentWatcher` class + `MisbehaviorReport`
  dataclass + per-category detector functions + `_BeadFiler` shared helper.
...
- More beads in the backlog (deferred, but still cluttering). Mitigated by
...
- Captures a class of failures that today only surfaces if an operator
  ...
  worktree, shell-as-tool-name, agents committing without `bd close`)
...
- Each filed bead with evidence is a closed-loop signal for prompt /
...
- Cross-run analysis (e.g. "this agent always loops on file Y" across many
  ...
  filed bead history and is a separate concern.
- Operator UI for bulk-resolving misbehavior beads. v1 is just the filing.
