...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 GET /api/v1/roles returns strategy and candidates for fast, standard, deep, and default.
- [ ] #2 PUT /api/v1/roles accepts multi-candidate role bodies and persists them.
- [ ] #3 PUT rejects an invalid candidate and leaves all existing roles unchanged.
- [ ] #4 PUT rejects an invalid strategy.
- [ ] #5 Provider and model status information is available for each candidate so the UI can show whether a candidate is usable.
- [ ] #6 Existing single-candidate behavior is either supported temporarily or explicitly updated in tests and UI in the same implementation series.
<!-- AC:END -->
...
## Definition of Done
<!-- DOD:BEGIN -->
- [ ] #1 API tests cover success, validation failure, and rollback behavior.
- [ ] #2 No direct edits to backlog task files are needed for this implementation.
<!-- DOD:END -->
